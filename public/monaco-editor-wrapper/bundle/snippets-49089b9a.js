import { B as o, aj as de, hy as ue, U as fe, aG as H, wj as m, bB as D, cx as ge, bE as W, aY as Se, bC as he, g as me, f as be, bD as we, cK as k, ao as Q, kJ as j, oU as ye, cP as ve, je as Ie, lg as R, wk as Ce, wl as xe, hj as ke, L as K, au as G, bi as V, fR as _, cO as Y, e as z, wm as Pe, aE as $e, vm as P, ml as Ee, _ as B, a as $, hM as Ae, aL as Te, w as X, ai as N, bz as Ne, N as q, hP as je, h$ as _e, wn as Le, pS as Me, qa as Fe, k as Oe, wo as Ue, qB as De, e_ as We, lm as Re } from "./index-4c11c534.js";
const Z = {
  category: {
...
async function ze(u, e, i, p) {
  ...
  return s.sort((t, a) => {
    const l = j(t.filepath.path), f = j(a.filepath.path);
...
async function J(u, e, i, p, s, r) {
  function n(t) {
    const a = j(t) !== ".code-snippets" ? `${t}.code-snippets` : t;
    return Q(e, a);
  }
...
class qe extends ee {
  ...
  async run(e) {
    const i = e.get(m), p = e.get(D), s = e.get(ge), r = e.get(W), n = e.get(Se), d = e.get(he), c = e.get(me), t = e.get(be), a = e.get(we), l = await ze(i, n, r, a), f = l.existing, S = [{
    ...
    if (h.indexOf(g) >= 0)
      return J(g.scope, g.uri, p, c, t, s);
    if (L.is(g))
      return g.hint && await Be(g, c, t), s.open(g.filepath);
  }
}
const E = class E extends ee {
  ...
  async run(e) {
    var t;
    const i = e.get(m), p = e.get(D), s = e.get(ve), r = e.get(W), n = Ie(s.activeTextEditorControl);
    if (!n || !n.hasModel())
      return;
...
class He extends te {
  ...
  async runEditorCommand(e, i, p) {
    ...
    const n = e.get(Y), d = e.get(z), c = await new Promise((l, f) => {
      const { lineNumber: S, column: h } = i.getPosition(), { snippet: g, name: b, langId: v } = M.fromUser(p);
      if (g)
        return l(new Pe(
          !1,
          [],
...
const T = class T extends te {
  constructor() {
    super({
      ...T.options,
      precondition: $e.and(_.writable, _.hasNonEmptySelection),
      f1: !0
    });
  }
...
N(A);
N(qe);
const Ke = q.as(je.Workbench);
Ke.registerWorkbenchContribution(U, 3);
q.as(_e.Configuration).registerConfiguration({
  ...Le,
  properties: {
...
}, re = q.as(Me.JSONContribution);
re.registerSchema(oe, Ge);
